...
import com.hapag.kubo.dao.InvoiceDAO;
...
//            closeConnection();
    ...
    public ArrayList<com.hapag.kubo.model.Invoice> getSalesUser(Long userId, Long startId, Long endId) {
        ArrayList<com.hapag.kubo.model.Invoice> list = new ArrayList<>();
        try {
            String query = "SELECT \n"
                    + "    USER_ACCOUNT.SURNAME, \n"
                    + "    USER_ACCOUNT.FIRSTNAME, \n"
            ...
            ResultSet rs = st.executeQuery(query);
            while (rs.next()) {
                com.hapag.kubo.model.Invoice i = new com.hapag.kubo.model.Invoice();
